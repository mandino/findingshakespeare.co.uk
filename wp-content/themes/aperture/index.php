...
<?php $GLOBALS['homepage'] = 'true';?>        
	...
	<div id="albums">    
		...
		<div class="container_16">
			...
			<?php $exclude = get_exclude_categories("woo_cat_box_");
				$getcats = get_categories('hierarchical=0&hide_empty=0&exclude='. $exclude);
				$track = array();
			foreach ( $getcats as $cat ) { ?>
			
			<?php $count = 0; $cat_id = $cat->cat_ID; query_posts("cat=$cat_id&showposts=100"); 
				if (have_posts()) : while (have_posts()) : the_post();
					if (!in_array($post->ID,$track) ) {                        ?>
					
					<div class="grid_4">
						<div class="category-image-block">
							<a href="<?php echo get_category_link($cat_id);?>"><?php                             
							$img = get_option('woo_cat_box_'. $cat_id .'_image');
                            if( $img ){
								woo_image('src=' .$img. '&height=150&width=220');
                            }
                            else {
