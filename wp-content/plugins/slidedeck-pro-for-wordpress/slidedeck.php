...
define( 'SLIDEDECK_VERSION',                        '1.3.72' );
...
define( 'SLIDEDECK_CUSTOM_SKIN_DIR', 			    dirname( dirname( __FILE__ ) ) . "/slidedeck-skins" );
...
function slidedeck_get_skins( $type = 'all' ) {
	...
	if( is_dir( SLIDEDECK_CUSTOM_SKIN_DIR ) ) {
		...
		foreach( (array) $custom_skin_files as $custom_skin_file ) {
			$custom_skin_slug = basename( dirname( $custom_skin_file ) );
			for( $i = 0; $i < count( $skin_files ); $i++ ) {
				$skin_slug = basename( dirname( $skin_files[$i] ) );
				if( $skin_slug == $custom_skin_slug ) {
					$skin_files[$i] = $custom_skin_file;
				}
			}
		}
	}
...
function slidedeck_save( $post_params = null, $post_status = 'publish' ) {
    ...
    if( $params['dynamic'] != '1' ) {
        ...
        foreach( (array) $params['slide'] as $slide ) {
            if( $slide['vertical'] == 1 ) {
                $params['slide'][$vertical_serialize_counter]['content'] = serialize( $slide['vertical_content'] );
            }
...
function slidedeck_process_template( $slidedeck_id, $styles = array( 'width' => '100%', 'height' => '300px' ) ) {
    ...
    if ( isset( $slidedeck ) && !empty( $slidedeck ) ) {
        ...
        if( slidedeck_has_vertical_slides( $slidedeck ) ) {
...
function slidedeck_output( $slidedeck, $slidedeck_uid, $slides, $styles_str ) {
    ...
    foreach ( (array) $slides as $slide ) {
        ...
        if( isset( $slide['background'] ) && !empty( $slide['background'] ) ) {
            $template_str.= '<dd style="background:url(' . $slide['background'] . ') 23px center !important;">';
        } else {
            $template_str.= '<dd>';
        }

        if( $is_vertical ) {
            $vertical_content = unserialize( $slide['content'] );
            $template_str.= '<ul class="slidesVertical">';
            foreach( $vertical_content as $vertical_slide ) {
                $template_str.= '<li>' . slidedeck_process_slide_content( $vertical_slide ) . '</li>';
            }
            $template_str.= '</ul>';
        } else {
